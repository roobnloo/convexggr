```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
source("data_generation.R")
source("gmmreg.R")
source("cggr.R")
source("test_utils.R")
# library(kableExtra)
library(abind)
```

```{r}
set.seed(6)
d <- 25
p <- 50
n <- 200
tB <- readRDS("data/tB.rds")
# tB <- generate_tb(d, p)
# mG <- matrix(runif(d * p, -0.25, 0.25), d, p) #generate_mg(d, p)
mG <- generate_mg(d, p)

generate <- function(n, d, p, tb, mg, seed) {
  set.seed(seed)
  s <- data_generate(n, d, p, tb, mg)
  return(s)
}
generate_datasets <- function(tbpath, mgpath, n, seed) {
    set.seed(seed)
    tb <- readRDS(tbpath)
    mg <- readRDS(mgpath)
    s <- data_generate(n, tb, mg)
    return(s)
}
```

```{r}
performance <- function(result, tb, s) {
  stopifnot(all(dim(result$bhat) == dim(tb)))
  stats <- list()
  stats$tpr <- sum(result$bhat != 0 & tb != 0) / sum(tb != 0)
  stats$fpr <- sum(result$bhat != 0 & tb == 0) / sum(tb == 0)

  beta_err <- 0
  for (i in 1:d) {
    beta_err <- beta_err + sqrt(sum((tb[i, , ] - result$bhat[i, , ])^2))
  }
  stats$beta_err <- beta_err / n

  mean_err <- 0
  for (i in 1:n) {
    mean_err <- mean_err + sum((s$mumx[i, ] -  result$mean(i))^2) / n
  }
  stats$mean_err <- mean_err / n


  omega_err <- 0
  iu <- cbind(0, s$U)
  for (i in 1:n) {
    omega <- apply(tb, c(1, 2), \(b) b %*% iu[i, ])
    omhat <- result$precision(i)
    diag(omhat) <- 0
    omega_err <- omega_err + sum((omega - omhat)^2) / n
  }
  stats$omega_err <- omega_err

  return(stats)
}
```

## Run methods
```{r}
trials <- 1
s_list <- vector("list", length = trials)
for (i in seq_along(s_list)) {
  s_list[[i]] <- generate(n, d, p, tB, mG, seed = i)
}
```


```{r}
gmmreg_results <- vector("list", length = trials)
# cggr_results <- vector("list", length = trials)
for (i in seq_along(s_list)) {
  s <- s_list[[i]]
  g_result <- gmmreg(s$X, s$U, 0.70, verbose = TRUE)
  # c_result <- cggr(s$X, s$U, 0.70, verbose = TRUE)
  tictoc::tic()
  gmmreg_results[[i]] <- g_result
  tictoc::toc()
  # cggr_results[[i]] <- c_result
  print(paste("Finished trial", i))
}
```

```{r}
gmmreg_stats <- matrix(nrow = trials, ncol = 4)
cggr_stats <- matrix(nrow = trials, ncol = 4)
for (i in seq_len(trials)) {
  gmmreg_stats[i, ] <- unlist(performance(gmmreg_results[[i]], tB, s_list[[i]]))
  cggr_stats[i, ] <- unlist(performance(cggr_results[[i]], tB, s_list[[i]]))
}

gmmreg_stats_mean <- apply(gmmreg_stats, 2, mean)
cggr_stats_mean <- apply(cggr_stats, 2, mean)
stats <- rbind(gmmreg_stats_mean, cggr_stats_mean)
colnames(stats) <- c("TPR", "FPR", "mean_err", "omega_err")
stats
```

```{r}
stats_gmmreg <- apply(gmmreg_stats, 2, mean)
stats_cggr <- apply(cggr_stats, 2, mean)
stats <- rbind(stats_gmmreg, stats_cggr)
colnames(stats) <- c("TPR", "FPR", "mean_err", "omega_err")
stats
# kable(as.data.frame(stats), digits = 3, padding = 10)  |>
#   kable_styling()
```


## Compare coefficient matrices
```{r}
for (i in seq_len(6)) {
  cov_lbl <- as.character(i - 1)
  beta_list <- list(GMMReg = gmmreg_results[[20]]$bhat[, , i],
                    CGGR = cggr_results[[20]]$bhat[, , i],
                    Actual = tB[, , i])
  print(beta_viz_list(beta_list, cov_lbl, tileborder = TRUE))
}
```

### Compare gamma
```{r}
print(gamma_viz_list(list(
  GMMReg = gmmreg_results[[1]]$ghat,
  CGGR = cggr_results[[1]]$ghat,
  Actual = mG
)))
```


```{r}
a_seq <- seq(0.1, 1, by = 0.1)
perf <- vector("list", length = 10)
s <- s_list[[3]]
for (i in 1:10) {
    cggr_res <- cggr(s$X, s$U, a_seq[i])
    perf[[i]] <- performance(cggr_res, tB, s)
}
Reduce(cbind, perf)
```

```{r}
for (i in seq_len(6)) {
    print(beta_viz_compare(result$bhat[, , i], tb[, , i], i))
}

```

# Matlab
```{r}
writeMat("GMMReg/GMMReg/gen.mat", resp = s$X, covar = s$U)
run_matlab_script(paste0(getwd(), "/GMMReg/GMMReg/testSerious.m"))
```

Using GMMReg (matlab)
```{r}
result <- readMat("GMMReg/GMMReg/genresult.mat")[[1]]
tuned <- readMat("GMMReg/GMMReg/tunedlambda.mat")$tune
tunedlambda <- tuned[,1]
tunedalpha <- tuned[,2]
resultlist <- apply(result, 3, function(t) t, simplify = F)

stats_matlab <- list()
stats_matlab$tpr <- sum(result != 0 & tB != 0) / sum(tB != 0)
stats_matlab$fpr <- sum(result != 0 & tB == 0) / sum(tB == 0)
stats_matlab$beta_err <- sqrt(sum((tB - result)^2))
# kable(as.data.frame(stats), digits = 3, padding = 10)  |>
#   kable_styling()
stats_matlab
```

### Compare GMMReg (matlab) component matrices
```{r compare components}
for (i in seq_len(6)) {
  cov_lbl <- paste0("component ", i - 1)
  print(beta_viz_compare(result[,,i], tB[,,i], cov_lbl, tileborder = T))
}
```
