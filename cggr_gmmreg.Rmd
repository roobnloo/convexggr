...
## Run methods
```{r}
gmmreg_result <- gmmreg(s$X, s$U, 0.75)
cggr_result <- cggr(s$X, s$U, asparse = 0.75)
...
for (i in seq_len(6)) {
  ...
  beta_list <- list(GMMReg = gmmreg_result$bhat[,,i],
                    CGGR = cggr_result$bhat[,,i],
                    Actual = tB[,,i])
...
print(gamma_viz_list(list(
  GMMReg = gmmreg_result$ghat,
  CGGR = cggr_result$ghat,
  Actual = mG
)))
```
