...
mG <- readRDS("data/mg_p25q100sparseTRUEsg500.rds")

generate <- function(n, tB, mG, seed) {
  set.seed(seed)
...
performance <- function(result, s_true) {
  ...
  stats$mtpr <- sum(result$ghat != 0 & mG != 0) / sum(mG != 0)
  stats$mfpr <- sum(result$ghat != 0 & mG == 0) / sum(mG == 0)
  mean_err <- 0
  for (i in 1:n) {
...
```{r run, cache=FALSE, cache.path="./cache/"}
gmmreg_result_mx <- matrix(nrow = trials, ncol = 7)
cggr_result_mx <- matrix(nrow = trials, ncol = 7)
for (i in seq_along(s_list)) {
  ...
  c_result <- cggr(
    s_list[[i]]$X, s_list[[i]]$U, 0.75, nregmean = 20, tol = 1e-6,
    verbose = TRUE, parallel = TRUE)
