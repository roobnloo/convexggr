...
```{r}
cggr_result <- cggr_node(s$responses[, component],
...
gamma_j <- Variable(p)
beta_j <- Variable((p + 1) * (d - 1))
cvxr_result <- cggr_node_cvxr(s$responses[, component],
...
```{r}
cggr_result <- cggr_node_warm(s$responses[, component],
                              s$responses[, -component],
                              s$covariates,
                              lambda_g = 0.03, lambda_b = 0.02,
                              alpha = 0.5,
                              runif(p), rep(0, (p + 1) * (d - 1)),
                              1000, 1e-5)
