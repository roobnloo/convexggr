...
p <- 50
num <- 200
s <- generate_data(num, d, p, ve = 0.01, sg = 0, reparametrize = F)
```
...
```{r}
sgl_result <- cggr_sgl(s$responses, s$covariates, 0.5, reparametrize = F)
```

```{r}
true_pos_rate(sgl_result$beta_mxs, s$b_mxs, 1:(length(s$b_mxs) - 1))
1-true_neg_rate(sgl_result$beta_mxs, s$b_mxs, s$cov_nz_idx)
```
